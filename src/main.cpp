...
#if defined(NDEBUG)
# error "Lukercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x07e1da3cae28219e029320a88312e471c50933a59752773f124b8093ed1fe702");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lukercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lukercoin Signed Message:\n";
...
{
    // Lukercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 400000); // Lukercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Lukercoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Lukercoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lukercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lukercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x835f79a4bb14cd4ff383e830c4859f2cfe628ccd35d1c31c68ddf1859c52ad80");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Trump Complains About Immigrants from Shithole Countries";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047e8c9445ee55ee6bddd13689b42a409d17ec365352cc1ab6dc94b0e9271abd7a4cffbf03714435f8914a136c9dd3807ada7db7fc5bb2970bb92233cc8a6c4288") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515860707;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084673449;

        if (fTestNet)
        {
            block.nTime    = 1515860667;
            block.nNonce   = 385610247;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x82c1ee4cef1dbb32c59ab163899b7c556ff41b797db7676c8414e2fa66c6d584"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc1, 0xb9, 0xd1 }; // Lukercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LukercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LukercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LukercoinMiner(CWallet *pwallet)
{
    printf("LukercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lukercoin-miner");
    ...
    try { loop {
        ...
        printf("Running LukercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LukercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LukercoinMiner, pwallet));
}
